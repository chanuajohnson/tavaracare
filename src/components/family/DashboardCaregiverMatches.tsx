                        ...
                        <div className="h-16 w-16 rounded-full border-2 border-primary/20 bg-primary/10 flex items-center justify-center overflow-hidden">
                          {cg?.avatar_url ? (
                            ...
                            <span className="text-lg font-semibold text-primary">
                              {cg.full_name ? cg.full_name.split(' ').filter(Boolean).map((p: string) => p[0]).join('').substring(0, 2).toUpperCase() : 'CG'}
                            </span>
                          )}
                        </div>

                        <div className="mt-2 text-center sm:text-left">
                          <h3 className="font-semibold">{cg.first_name || cg.full_name?.split(' ')[0] || 'Caregiver'}</h3>
