...
export const ChatInputForm: React.FC<ChatInputFormProps> = ({
  ...
  const getPlaceholder = (): string => {
    ...
    if (isFocused) {
      ...
      if (fieldType === "phone") return "Enter your phone number";
      if (fieldType === "name") return "Enter your name";
      if (fieldType === "budget") return "Enter budget";
      return "Type your message...";
    }
    
    // More detailed guidance for unfocused state
    if (fieldType === "email") {
      return "Enter your email (e.g., name@example.com)";
    }
    
    if (fieldType === "phone") {
      return "Enter phone number (e.g., +1 868 123 4567)";
    }
    
    if (fieldType === "name") {
      return "Enter your name (First Last)";
    }
    
    if (fieldType === "budget") {
      return "Enter budget range (e.g., $20-30/hour)";
    }
  ...
  return (
    <div className={`border-t ${validationError ? "border-red-300 bg-red-50" : "border-border"} ${isMobile ? "px-2 py-1" : "p-2"} flex flex-col safe-bottom`}>
      ...
      {validationError && (
        <p id="input-error-message" className="text-xs text-red-500 mt-1">{validationError}</p>
      )}
    </div>
  );
};
