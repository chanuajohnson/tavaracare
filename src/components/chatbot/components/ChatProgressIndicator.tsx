...
export const ChatProgressIndicator: React.FC<ChatProgressIndicatorProps> = ({
  ...
  return (
    <div className={`w-full px-4 py-2 border-b border-gray-200 bg-white/95 sticky top-0 z-10 shadow-md`}>
      <div className="flex justify-between items-center mb-1.5">
        <div className="flex items-center">
          <span className={`font-medium ${isMobile ? "text-xs" : "text-sm"} text-primary-700`}>
            Section {sectionIndex + 1} of {totalSections}
          </span>
          <span className={`text-gray-600 ml-2 ${isMobile ? "text-xs" : "text-sm"} font-medium`}>
            • {sectionName}
          </span>
        </div>
        <span className={`text-primary-700 font-medium ${isMobile ? "text-xs" : "text-sm"}`}>{totalProgress}%</span>
      </div>
      <Progress value={totalProgress} className={`h-2 ${isMobile ? "w-full" : "w-full"} bg-gray-200`} />
    </div>
  );
};
