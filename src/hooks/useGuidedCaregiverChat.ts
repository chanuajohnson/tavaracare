...
export const useGuidedCaregiverChat = ({ caregiverId, caregiver }: UseGuidedCaregiverChatProps) => {
  ...
  // Initialize conversation with proper persistence integration - MEMOIZED
  const initializeConversation = useCallback(async () => {
    // Prevent multiple simultaneous initializations
    if (isInitializedRef.current || initializationPromiseRef.current) {
      console.log('[useGuidedCaregiverChat] Already initialized or initializing, skipping...');
      return initializationPromiseRef.current;
    }

    console.log(`[useGuidedCaregiverChat] *** STARTING INITIALIZATION ***`);
  ...
  // Handle prompt selection with enhanced persistence - MEMOIZED
  const handlePromptSelection = useCallback(async (promptText: string) => {
    ...
    console.log(`[useGuidedCaregiverChat] *** PROMPT SELECTION ***`);
  ...
  // Enhanced caregiver response checking with persistence integration - MEMOIZED
  ...
  // Reset initialization state when caregiverId changes
  useEffect(() => {
