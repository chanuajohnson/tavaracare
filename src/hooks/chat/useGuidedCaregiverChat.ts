  ...
  const initializeConversation = useCallback(async () => {
    ...
    try {
      ...
      if (flow) {
        ...
        // Add welcome message
        setMessages([{
          content: `💙 Hi! I'm TAV, your care coordinator. I'll help you connect with this amazing caregiver in a safe and structured way. Choose how you'd like to start the conversation:`,
          isUser: false,
          timestamp: Date.now()
        }]);
      }
    ...
    try {
      ...
      if (chatRequest && chatRequest.status === 'accepted') {
        ...
        if (conversationFlow) {
          ...
          setMessages(prev => [...prev, {
            content: '🎉 Great news! The caregiver has accepted your request and is ready to chat. What would you like to ask them?',
            isUser: false,
            timestamp: Date.now()
          }]);
        }
