...
DECLARE
  ...
  -- Family journey: 13 non-optional steps (includes the Care Readiness Check)
  total_steps integer := 13;
  ...
  readiness_complete boolean := false;
...
BEGIN
  ...
  ELSE
    ...
    -- Care Readiness Check (family pacing questionnaire)
    IF (user_record.family_readiness_profile IS NOT NULL
        AND user_record.family_readiness_profile::jsonb <> '{}'::jsonb)
       OR user_record.client_stage IS NOT NULL THEN
    ...
    completion_percent := ROUND((LEAST(step_count, total_steps)::numeric / total_steps::numeric) * 100, 0);
    current_step := LEAST(step_count + 1, total_steps);
  END IF;
...
DECLARE r record;
BEGIN
  FOR r IN SELECT id FROM profiles WHERE role = 'family' LOOP
    BEGIN
      PERFORM public.calculate_and_update_journey_progress(r.id);
